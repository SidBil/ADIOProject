#!/usr/bin/env python3
"""
Regenerate images 1-20 at 9:10 portrait ratio.

Strategy:
  1. Generate at 1024x1536 (gpt-image-1 portrait size)
  2. Center-crop height 1536 → 1138 for ~9:10 (1024x1138)
  3. Save as img_001.png … img_020.png, overwriting existing files

Run from imagegen/:
  source ../web/venv/bin/activate && python gen_batch_910.py
"""
...
TARGET_SIZE = (1024, 1138)  # ~9:10


def needs_regen(path: Path) -> bool:
    """Return True if file is missing or not already 9:10."""
    if not path.exists():
        return True
    import struct
    ...
    with open(CSV_FILE, encoding="utf-8") as f:
        for i, row in enumerate(csv.DictReader(f), 1):
            ...
            if needs_regen(path):
                ...
                print(f"  ⊙ img_{i:03d}.png already 9:10, skipping")

    if not tasks:
        print("All images already at 9:10. Nothing to do.")
        return

    print(f"\nGenerating {len(tasks)} image(s) at 9:10 (1024×1138) with {MAX_WORKERS} workers…\n")
